		...
		if ( $order && in_array( $order->get_status(), array( 'processing', 'completed' ), true ) ) {
			...
			if ( $product ) {
				if ( is_array( $requests )
					...
					echo '<button class="devolution-item-requested" title="Devolução Solicitada"  disabled="disabled">';
					echo '<span class="dashicons dashicons-hourglass"></span>';
					echo '</button>';
				} elseif ( ! is_admin() ) {
					echo '<button title="Solicitar Devolução" class="devolution-item-request" data-order_id="' . esc_attr( $order->get_id() ) . '" data-product_id="' . esc_attr( $product->get_id() ) . '">';
...
new Virtuaria_Correios_Devolutions();
